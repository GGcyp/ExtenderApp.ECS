...
public static class CustomRunnerCommandsVerification
{
    ...
    public static void VerifySharedComponentCrud()
    {
        ...
        // 注意：共享组件类型不应与实体组件类型复用（否则会在测试套件顺序变化时引入额外耦合）。
        Assert.True(world.TryAddSharedComponent(new ManagedSharedData() { Level = 1, Name = "s" }));

        Assert.True(world.TryGetSharedComponent<ManagedSharedData>(out var d));
    ...
    private sealed class ManagedSharedData
    {
        public string? Name { get; set; }
        public int Level { get; set; }
    }
