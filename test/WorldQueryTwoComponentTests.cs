...
public sealed class WorldQueryTwoComponentTests
{
    ...
    [Fact]
    public void Query_TwoComponents_CountAndMutationAndNone()
    {
        using var world = new World();
        world.CreateEntity(
            new TestPosition { X = 1, Y = 2 },
        ...
        var q = world.With<TestPosition, TestVelocity>()
                                                    .WithNone<TestHealth>()
                                                    .Build();
        Assert.Equal(2, q.Count);
        ...
        Assert.Equal(1f + 100f + 3f + 100f + 1f, sumX);
        Assert.Equal(10f + 1f + -1f + 1f + 10f, sumDx);
    }
}