...
namespace ExtenderApp.ECS.Accessors
{
    ...
    ///
    /// 语义：调用方可以按块序号（0 表示列表的第一个块）直接获取对应块的单块访问器 <see cref="ComponentAccessor{T}" /> 或按块级别枚举所有块进行批量处理。该类型不会遍历块内元素，仅负责块级定位与迭代。
    /// </summary>
    /// <typeparam name="T">组件类型（值类型且实现 <see cref="IComponent" />）。</typeparam>
    internal struct ArchetypeChunkAccessor<T> where T : struct
    {
        ...
        /// <summary>
        /// 返回列表中块的数量（块节点数）。若未提供列表则返回 0。 注意：此数量表示块节点数，不等同于块内元素总数。
        /// </summary>
        ...
        /// <summary>
        /// 获取块级枚举器，用于按块顺序遍历所有存在的块。 枚举器为 struct，可避免堆分配，适合在热路径中循环处理每个块。
        /// </summary>
        /// <returns>返回一个 <see cref="Enumerator" /> 实例，用于块级遍历。</returns>
        public Enumerator GetEnumerator() => new Enumerator(_chunks, _version);

        /// <summary>
        /// 块级枚举器：每次 MoveNext 前进到下一个可用块（从索引 0 开始）， Current 返回当前块对应的 <see cref="ComponentAccessor{T}" />。
        /// </summary>
