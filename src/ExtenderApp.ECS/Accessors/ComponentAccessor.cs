...
namespace ExtenderApp.ECS.Accessors
{
    ...
    ///
    /// 语义：当调用方已经知道某列的组件类型为 <c>T</c> 时，使用此类型可以直接
    /// 从对应的 <see cref="ArchetypeChunk{T}"/> 读取组件副本或创建对组件的引用包装（<see cref="RefRO{T}"/> / <see cref="RefRW{T}"/>），
    /// 避免在热路径中频繁进行运行时类型转换或产生堆分配。
    /// </summary>
    /// <typeparam name="T">组件类型（值类型，且实现 <see cref="IComponent"/>）。</typeparam>
    public readonly struct ComponentAccessor<T> where T : struct, IComponent
    {
        /// <summary>
        ...
        /// <summary>
        /// 使用指定的 <see cref="ArchetypeChunk{T}"/> 创建访问器实例。
        /// 请确保传入的 current 类型与 T 对应且已被正确初始化（已调用 Initialize）。
        /// </summary>
        ...
        /// <param name="index">块内的局部索引（0-based）。</param>
        /// <returns>
        /// 返回对应位置的 <see cref="RefRW{T}"/>。
        /// </returns>
        public RefRW<T> GetRefRW(int index) => new(_chunk, index);

        /// <summary>
        /// 获取结构体枚举器，用于遍历该单个块内的所有现有元素（不跨块链）。
        /// 返回的枚举器为 struct，可与 foreach 一起使用以避免堆分配。
        /// </summary>
        ...
        /// <summary>
        /// 组件枚举器：按单块内顺序枚举当前块中所有已占用的槽位，并返回对应元素的副本（值语义）。
        /// 注意：在持有返回的引用期间不要对底层 Chunk 做结构性修改（如归还、释放或重新初始化），否则可能导致悬挂引用或未定义行为。
        /// </summary>
