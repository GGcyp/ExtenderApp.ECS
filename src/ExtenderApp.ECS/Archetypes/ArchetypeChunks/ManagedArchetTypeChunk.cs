...
namespace ExtenderApp.ECS.Archetypes
{
    ...
    ///
    /// 说明：此实现使用托管数组作为底层存储并可从 <see cref="ArrayPool{T}" /> 获取缓冲区以减少分配开销。 它提供对单个组件的读写与引用访问，并实现块内交换与跨块复制等操作。
    /// </summary>
    internal sealed class ManagedArchetTypeChunk<T> : ArchetypeChunk<T>
    {
        ...
        /// <summary>
        /// 返回当前块内组件的连续内存视图（Span&lt;T1&gt;）。 视图直接基于内部数组，访问需保证块已初始化并且索引在有效范围内。
        /// </summary>
        ...
        /// <summary>
        /// 构造函数：允许指定用于分配数组的 <see cref="ArrayPool{T}" />。 初始状态下内部数组为空数组，实际缓冲区在初始化时从池租用。
        /// </summary>
        ...
        /// <summary>
        /// 将外部内存的不安全复制（native 来源）复制到本块指定位置，用于高性能拷贝场景。 当前尚未实现，调用将抛出 <see cref="NotImplementedException" />。
        /// </summary>
        ...
        /// <summary>
        /// 在块内交换两个本地索引处的组件值。 已实现为通过元组交换数组元素。
        /// </summary>
        ...
        /// <summary>
        /// 尝试将当前块中 globalIndex 对应的组件复制到目标 ArchetypeChunk 的 newGlobalIndex。 当目标块类型匹配且对应索引均在各自块内时执行复制并返回 true，否则返回 false。
        /// </summary>
