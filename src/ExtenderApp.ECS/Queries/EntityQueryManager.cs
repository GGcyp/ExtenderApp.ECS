...
namespace ExtenderApp.ECS.Queries
{
    ...
    ///
    /// 线程语义：该类型不是线程安全的，通常每个 CurrentWorld 持有一个实例并在主线程上使用；若在多线程场景下访问，需要外部同步。
    /// </summary>
    internal class EntityQueryManager
    {
        private readonly Func<EntityQueryDesc, EntityQueryCore> _coreFactory;

        /// <summary>
        /// 当前 CurrentWorld 中的全部 Archetype 注册表（由 Archetype 仓库维护），用于查询核心的匹配阶段遍历。
        /// </summary>
        private readonly ArchetypeRepository _archetypeRepository;

        /// <summary>
        /// CurrentWorld 版本管理器。用于在查询核心中判断匹配缓存是否失效并触发重建。
        /// </summary>
        ...
        /// </summary>
        private readonly ConcurrentDictionary<EntityQueryDesc, EntityQueryCore> _queries;

        /// <summary>
        /// 创建查询管理器实例。通常由 CurrentWorld 在初始化时构造并持有生命周期。
        /// </summary>
        /// <param name="archetypeRepository">当前 CurrentWorld 中全部 Archetype 的只读集合。</param>
        /// <param name="worldVersionManager">CurrentWorld 版本管理器。</param>
        public EntityQueryManager(ArchetypeRepository archetypeRepository, WorldVersionManager worldVersionManager)
        {
            ...
            _queries = new();
            _coreFactory = desc => new(_archetypeRepository, desc, _worldVersionManager);
        }
        ...
        public EntityQueryCore GetOrCreateCore(EntityQueryDesc desc)
        {
            ...
            return _queries.GetOrAdd(desc, _coreFactory);
        }
    }
}