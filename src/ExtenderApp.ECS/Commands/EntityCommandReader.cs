    ...
    internal class EntityCommandReader
    {
        ...
        private void ApplyRemoveComponent(Entity target, ComponentType ct)
        {
            ...
            if (newMask.IsEmpty)
            {
                ...
                _entityManager.TryChangedArchetype(target, null, -1);
                return;
            }

            var newArchetype = _archetypeManager.GetOrCreateArchetype(newMask);
            var newIndex = newArchetype.AddEntity(target);
            FinishArchetypeMigrationAfterReserveSlot(target, archetype, archetypeIndex, newArchetype, newIndex);
        }
        ...
        private void ApplyModifyComponent(Entity target, ComponentMask mask)
        {
            ...
            var newIndex = newArchetype.AddEntity(target);
            _entityManager.TryGetArchetype(target, out var oldArchetype, out var oldArchetypeIndex);
            FinishArchetypeMigrationAfterReserveSlot(target, oldArchetype, oldArchetypeIndex, newArchetype, newIndex);
        }
        ...
        /// <param name="oldArchetypeIndex">在旧原型中的全局行号；无旧原型时忽略。</param>
        /// <param name="newArchetype">已预留槽位的新原型。</param>
        /// <param name="newIndex">在新原型中的全局行号。</param>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void FinishArchetypeMigrationAfterReserveSlot(
            Entity target,
            Archetype? oldArchetype,
            int oldArchetypeIndex,
            ...
            int newIndex)
        {
            ...
            if (oldArchetype != null)
            {
                if (newArchetype != null &&
                    newIndex >= 0 &&
                    oldArchetype.TryCopyToAndRemove(oldArchetypeIndex, newArchetype, newIndex, out changedEntity))
                {
                    if (!changedEntity.IsEmpty)
                        _entityManager.TryChangedArchetypeIndex(changedEntity, oldArchetypeIndex);
                }
                else if (oldArchetype.TryRemoveEntity(oldArchetypeIndex, out changedEntity))
                {
                    if (!changedEntity.IsEmpty)
                        _entityManager.TryChangedArchetypeIndex(changedEntity, oldArchetypeIndex);
                }
