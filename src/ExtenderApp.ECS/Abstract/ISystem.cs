...
namespace ExtenderApp.ECS.Systems
{
    ...
    /// 系统基础接口（生命周期钩子）。
    /// 
    /// 语义：实现该接口的类型表示一个可被世界（World）调度的系统。系统应只包含业务逻辑或调度作业的代码，
    /// 不应直接承担跨系统的同步或回放命令缓冲的职责。生命周期方法由框架在适当的线程/阶段调用，
    /// 并通过上下文对象传入运行时所需的最小资源（例如创建时的服务、更新时的时间信息与命令缓冲等）。
    /// </summary>
    public interface ISystem
    {
        /// <summary>
        /// 系统创建阶段调用（主线程）：用于初始化长期资源、构建查询与注册依赖。
        /// 注意：该方法在主线程执行；若需要并行准备，应只调度任务并通过组级同步在后续阶段等待其完成。
        /// </summary>
        ...
        /// <summary>
        /// 系统销毁阶段调用（主线程）：用于释放长期资源与执行最终清理。
        /// 注意：释放涉及主线程资源的操作应在该方法内完成。
        /// </summary>
        void OnDestroy();
    }
}