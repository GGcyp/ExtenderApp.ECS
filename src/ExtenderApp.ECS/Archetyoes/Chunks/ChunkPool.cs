...
namespace ExtenderApp.ECS.Archetypes
{
    ...
    internal sealed class ChunkPool : DisposableObject
    {
        ...
        /// <summary>
        /// 池中允许保留的最大块数量（包含 lastChunk 与 queue 中的项）。
        /// 超过该数量后，归还的 Chunk 将被释放而不是入池。
        /// </summary>
        private const int MaxPoolSize = 10;

        /// <summary>
        /// 后备的线程安全队列，存储空闲的 Chunk 实例。
        /// 延迟初始化以减少不必要分配。
        /// </summary>
        private ConcurrentQueue<Chunk> queue;

        /// <summary>
        /// 一个无锁快速路径的单槽缓存（可由任一线程原子交换），用于减少入/取操作的开销。
        /// 当有新请求时先尝试从该槽获取；归还时优先放入该槽。
        /// </summary>
        private Chunk? lastChunk;

        /// <summary>
        /// 当前池中近似的可用 Chunk 数量（包括 lastChunk 与队列中的数量）。
        /// 仅供监控/调试使用，不保证严格精确性（并发条件下为近似值）。
        /// </summary>
