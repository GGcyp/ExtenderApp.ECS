...
{
    /// <summary>
    /// 非泛型基类：用于按组件类型查找或创建对应的 <see cref="ArchetypeChunkProvider{T}"/> 实例。
    /// 定义了租用与归还 <see cref="ArchetypeChunk"/> 的抽象接口，具体实现由泛型子类负责。
    /// </summary>
    internal abstract class ArchetypeChunkProvider
    {
        /// <summary>
        /// 缓存每个组件类型对应的提供器实例，避免反射重复创建。
        /// 键为组件类型（struct 类型实现 IComponent），值为对应的 ArchetypeChunkProvider 实例。
        /// </summary>
        ...
        /// <summary>
        /// 根据组件类型查找或创建对应的 ArchetypeChunkProvider 实例。
        /// 组件类型必须为值类型（struct）并实现 <see cref="IComponent"/> 接口。
        /// 返回的实例为对应的泛型子类（<see cref="ArchetypeChunkProvider"/>）。
        /// </summary>
        /// <param name="componentType">组件类型（例如 typeof(MyComponent)）。</param>
        /// <returns>对应的 ArchetypeChunkProvider 实例（不能为 null）。</returns>
        public static ArchetypeChunkProvider GetOrCreate(Type? componentType)
        {
            ...
            if (!componentType.IsValueType || !typeof(IComponent).IsAssignableFrom(componentType))
                throw new ArgumentException("组件类型必须为值类型（struct）并实现 IComponent 接口", nameof(componentType));
        ...
        /// 类型安全的泛型获取或创建方法。
        /// </summary>
        /// <typeparam name="T">组件类型（struct, IComponent）。</typeparam>
        /// <returns>对应的 <see cref="ArchetypeChunkProvider{T}"/> 单例实例。</returns>
        public static ArchetypeChunkProvider<T> GetOrCreate<T>() where T : struct, IComponent
        {
        ...
        public abstract ArchetypeChunkList CreateChunkList();
    }
    ...
    internal sealed class ArchetypeChunkProvider<T> : ArchetypeChunkProvider where T : struct, IComponent
    {
        /// <summary>
        /// 池允许保留的最大 ArchetypeChunk 实例数（含快速槽与队列中的项）。
        /// 超过该数量后归还的实例将被释放。
        /// </summary>
        ...
        /// <summary>
        /// 获取池中近似可用的 ArchetypeChunk 数量（包括快速槽与队列中的项）。
        /// 并发环境下为近似值，仅用于监控或调试。
        /// </summary>
        ...
        /// 2. 若无则从队列弹出；
        /// 3. 若队列也为空则创建新的实例并返回。
        /// 返回的对象 <see cref="ArchetypeChunk{T}.StartIndex"/> 会被设置为传入的 <paramref name="startIndex"/> 值，
        /// 但不会自动 Initialize 底层 Chunk（由调用者决定何时调用 Initialize）。
        /// </summary>
        ...
        /// <summary>
        /// 将不再使用的 <see cref="ArchetypeChunk{T}"/> 实例归还到池中。
        /// 行为：
        ...
        public override ArchetypeChunkList<T> CreateChunkList() => new();
    }
}