...
namespace ExtenderApp.ECS.Queries
{
    ...
    ///
    /// 说明：组件列的位置不会在此处固定缓存（以避免不同 Archetype 上列位置差异）， 组件列的动态解析由访问器层（如 <see cref="ArchetypeAccessor{T}" />）在运行时完成。 此类型只负责匹配与维护匹配集合的生命周期与版本检测。
    /// </summary>
    internal sealed class EntityQueryCore
    {
        ...
        /// <summary>
        /// 获取当前匹配的 Archetype 总数。该属性会遍历匹配的 Archetype 链表并累加每个 Archetype 中的实体数量。
        /// </summary>
        ...
        /// <summary>
        /// 为指定组件类型创建按 Archetype 聚合的访问器。 返回的 <see cref="ArchetypeAccessor{T}" /> 可用于跨匹配 Archetype 遍历该组件的列数据。
        /// </summary>
        ...
        internal GlobalQueryEnumerator<T> GetArchetypeEnumerator<T>(bool skipUnchanged) => new(GetArchetypeAccessor<T>(skipUnchanged));

        /// <summary>
        /// 获取实体枚举器（按匹配的 Archetype 遍历实体句柄）。 该枚举器可与组件访问器配合使用以生成按行的查询结果。
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal GlobalQueryEntityEnumerator GetArchetypeEntityEnumerator() => new(GetArchetypeEntityAccessorEnumerator());

        /// <summary>
        /// 获取原型访问器枚举器。该访问器枚举器会根据当前匹配的 Archetype 链表和版本参数进行初始化，以支持跨 Archetype 的组件列访问。 版本参数的值由 <see cref="GetVersion(bool)" /> 方法根据是否启用跳过未变化块的优化逻辑动态确定。
        /// </summary>
        /// <typeparam name="T">组件类型。</typeparam>
        /// <param name="skipUnchanged">是否启用跳过未变化块的优化。</param>
        /// <returns>返回相应类型的 <see cref="ArchetypeAccessorEnumerator{T}" />。</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        /// <summary>
        /// 获取原型实体访问器枚举器。该枚举器会根据当前匹配的 Archetype 链表进行初始化，以支持跨 Archetype 的实体句柄访问。 由于实体访问器不涉及版本检测，因此不需要传入版本参数。
        /// </summary>
        /// <returns>返回相应类型的 <see cref="ArchetypeEntityAccessorEnumerator" />。</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        /// <summary>
        /// 根据是否跳过未变化块返回访问器应使用的版本参数。 若 <paramref name="skipUnchanged" /> 为 true，则返回当前世界版本号以启用跳过逻辑；否则返回 0 表示不启用跳过。
        /// </summary>
        ...
        {
            if (lastArchetypeCount == _repository.Count)
                return;

            RebuildMatches();
            lastArchetypeCount = _repository.Count;
        }

        /// <summary>
        /// 根据查询描述遍历仓库中自上次扫描以来新增的 Archetype，并筛选出满足条件的 Archetype， 将匹配结果以链表形式追加到内部缓存中。
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RebuildMatches()
        {
            foreach (var archetype in _repository.GetArchetypeSpan(lastArchetypeCount))
            {
