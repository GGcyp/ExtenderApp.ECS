...
namespace ExtenderApp.ECS.Abstract
{
    ...
    ///
    /// 提供便捷的方法：创建实体、批量创建实体、获取或构建实体查询（EntityQuery/EntityQueryBuilder）、以及访问共享组件。 注意：OnCreate 通常在主线程执行。本上下文为轻量包装，不建议长期保存；若需长期访问 World，建议在 OnCreate 中缓存必要的查询或句柄。
    /// </summary>
    public readonly struct SystemCreateContext
    {
        ...
        /// <summary> 批量创建实体并将结果写入指定跨度（用于高效批量创建）。等同于调用 World.CreateEntity(Span<Entity>)。 </summary>
        ...
        {
            lookup = _world.GetEntityComponentLookup(entity);
