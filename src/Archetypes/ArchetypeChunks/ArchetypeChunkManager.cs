...
namespace ExtenderApp.ECS.Archetypes
{
    ...
    internal sealed class ArchetypeChunkManager
    {
        ...
        /// <summary>
        /// 从列尾回收连续空块并归还提供器；由 <see cref="CompactTrailingStorageAfterRemovals" /> 在实体段尾扫之后统一调用。
        /// </summary>
        /// <param name="columnIndex"> 列索引。 </param>
        private void RemoveEmptyChunksAtColumnTail(int columnIndex)
        {
            if (!TryGetChunkListForColumn(columnIndex, out var chunkList))
                return;
        ...
        /// <summary>
        /// 在单次或批量移除、复制迁移移除等写路径末尾调用：先压缩实体段列表尾部空段，再对各组件列做尾块回收，保证段下标与列块列表对齐。
        /// </summary>
        internal void CompactTrailingStorageAfterRemovals()
        {
            Entities.RemoveEmptySegments();
            for (int columnIndex = 0; columnIndex < _columns.Length; columnIndex++)
                RemoveEmptyChunksAtColumnTail(columnIndex);
        }
